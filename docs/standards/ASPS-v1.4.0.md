# ARUSHAI Standard Project Structure (ASPS) v1.4.0
...
├── config/
├── docs/
├── specs/                  # Feature specifications (OSD 4.13)
└── tests/
```
...
### 5.6 Specs Directory Standard

All MEDIUM and HEAVY tier projects include `specs/` at the project root.

```
specs/
├── templates/              # Reusable templates (brief, spec, plan, tasks)
├── F001-{feature-name}/
│   ├── spec.md
│   ├── plan.md
│   └── tasks.md
...
- Feature directories numbered sequentially (F001, F002...) with kebab-case names.
- Each directory contains the three spec documents generated by the spec-writer agent.
- Templates in `specs/templates/` are project-specific adaptations of the standard format.
- See OSD Section 4.13 for spec content requirements, approval gates, and lifecycle rules.
...
**Required:** CLAUDE.md (<150 lines), README.md, {project}_context.md, .claude/skills/ (3-10 skills), docs/context_archive.md, config/ (with default.yaml and README.md), specs/ directory, spec-writer agent, docs/arushai-os/ submodule
...
**Required:** CLAUDE.md (<200 lines), README.md, {project}_context.md, .claude/skills/ (10-25 skills), .claude/hooks/, docs/context_archive.md, docs/decisions/, docs/runbooks/, subdirectory CLAUDE.md files for each component, config/ (with default.yaml, README.md, and domain-specific configs), specs/ directory, spec-writer agent, docs/arushai-os/ submodule
**Optional:** .claude/agents/, .claude/commands/
...
| MEDIUM | spec-writer (mandatory) + code-reviewer (mandatory) + 1-2 project-specific agents |
| HEAVY | spec-writer (mandatory) + code-reviewer (mandatory) + 2-3 project-specific agents |
...
### 7.9 Mandatory Agent: spec-writer

Every MEDIUM and HEAVY tier project MUST have a `spec-writer` agent. This is the specification gate.

**Configuration:**
- **Tools:** Read, Grep, Glob, WebFetch, WebSearch (read-only)
- **Model:** sonnet (or inherit)
- **Invocation:** Three-phase: specify → plan → tasks. Founder approval between each phase.
- **References:** sdd-workflow skill, osd-compliance skill

**#1 mandate: ASK QUESTIONS when the brief is ambiguous. Never assume. This is non-negotiable.**

The spec-writer receives a Feature Brief from the founder and generates spec.md, plan.md, and tasks.md in three separate invocations. The founder reviews and approves at each phase boundary before the next phase begins.

See OSD Section 4.13.3 for full mandatory behaviors and agent definition template.
...
| specs/templates/ | — | Required | Required |
| .claude/agents/spec-writer.md | — | Required | Required |
| docs/arushai-os/ (submodule) | — | Required | Required |
...
### 9.4 Engineering Standards Cross-Reference

All ARUSHAI projects must comply with the 15 Engineering Standards principles defined in OSD Section 4.9. These are non-negotiable regardless of tier, though enforcement level varies by tier as specified in the OSD enforcement table.

The 15 principles organized by category:

**Foundation (day-one requirements):**
1. Externalized Configuration
2. Structured Logging from Day One
3. Config Directory Standard
4. Testing — No Code Without Tests
5. Error Handling — Standardized Patterns

**Quality (code health):**
6. Input Validation — Validate at Every Boundary
7. Type Safety
8. DRY + Single Responsibility
9. Dependency Management — Pin and Audit
10. Code Review — Every Change Reviewed

**Resilience (production-ready):**
11. Observability — Health Checks + Metrics + Alerts
12. CI/CD Quality Gates
13. Database Migration Discipline

**Scale (growth-ready):**
14. API Versioning — Version From Day One
15. Environment Parity

The `config/` directory is already included in all composition patterns (Sections 5.1–5.4) and is enforced in the `.gitignore` standard (Section 10). The scaffold skill creates the initial config directory structure with template files. The code-reviewer agent template enforces compliance with all applicable principles on every code review.
...
### 9.6 ARUSHAI Standards Reference

All MEDIUM and HEAVY tier projects include `arushai-os` as a git submodule at `docs/arushai-os/`.

- Provides canonical reference for OSD, ASPS, and AI Session Protocol.
- Skills derived from these documents must note the source version in the SKILL.md header.
- When the submodule updates, derived skills must be regenerated to reflect current standards.
- Setup: `git submodule add git@github.com:arushai-hq/arushai-os.git docs/arushai-os`
...
| 1.3.0 | 2026-03-16 | Cross-reference to OSD v1.7.0 Engineering Standards (15 principles). Updated code-reviewer template with full principle checklist. |
| 1.4.0 | 2026-03-19 | Added specs/ directory standard, spec-writer mandatory agent, arushai-os submodule standard. Refs OSD 4.13. |
